...
export const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o.setName("name").setDescription("역할의 이름").setRequired(true),
    )
    .addStringOption((o) =>
      o
        .setName("color")
        .setDescription("역할의 색상 (HEX Color)")
        .setRequired(true),
    ),
  func: async (interaction) => {
    ...
    const conn = pool();
    const [rows]: [RequestRole[], FieldPacket[]] = await conn.query<
      RequestRole[]
    >("SELECT * FROM RequestRole WHERE guildId = ?", [interaction.guildId]);

    if (!rows[0] || !rows[0].requestChannelId || !rows[0].reviewChannelId) {
      await interaction.editReply({
        content:
          "역할 요청 채널, 역할 요청 관리 채널이 설정되어있지 않아\n역할 요청을 할 수 없습니다.",
      });
      return;
    }
